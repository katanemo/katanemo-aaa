...
log katutil init-service --service_name 'patient records service' --service_description 'service to manage patient and diagnostic records' --api_spec $API_SPEC --redirect_uri www.google.com --print_output True --token XXXXX
EHR_SERVICE_ID=$(katutil init-service --service_name 'patient records service' --service_description 'service to manage patient and diagnostic records' --api_spec $API_SPEC --redirect_uri www.google.com --print_output True --token $EHR_ADMIN_TOKEN | jq -r .serviceId)
